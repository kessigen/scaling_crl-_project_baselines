"""Evaluate a trained image-based SAC policy. Runs locally on Windows (no EGL setup)."""
...
# EGL only matters on headless Linux; on Windows MuJoCo uses native GL fine.
...
def main():
    ...
    parser.add_argument("--no-randomize", action="store_true", help="Disable domain randomization at eval")
    parser.add_argument("--obs-camera", choices=WX250PickPlaceImageEnv.OBS_CAMERAS, default="front", help="Observation camera to render for evaluation.")
    parser.add_argument("--record-gif", type=str, default=None,
                        help="If set, record one rollout to this .gif path")
    parser.add_argument(
        "--record-gif-mode",
    ...
    for ep in range(args.n_episodes):
        ...
        print(f"  ep {ep+1:3d}: return={R:7.2f}  steps={steps:3d}  success={success}")
