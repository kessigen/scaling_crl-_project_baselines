...
def make_env(asset_root, render_mode=None):
    def _thunk():
...
def main():
    ...
    parser.add_argument("--run-dir", default="runs/wx250_pose_sac")
    parser.add_argument(
        "--drive-dir",
        default=None,
    ...
    resume_step = None
    if args.resume:
        ckpts = glob.glob(str(run_dir / "checkpoints" / "wx250_pose_sac_*_steps.zip"))
        def _ckpt_step(p):
    ...
    checkpoint_cb = CheckpointCallback(save_freq=135_000, save_path=str(run_dir / "checkpoints"), name_prefix="wx250_pose_sac", save_replay_buffer=True)
