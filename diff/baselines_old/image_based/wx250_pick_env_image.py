...
<mujoco model="wx250_pick_place_image">
  ...
  <worldbody>
    ...
    <body name="goal" pos="{goal_x} {goal_y} 0.058">
      <geom name="goal_marker" type="box" size="0.05 0.05 0.001" contype="0" conaffinity="0" material="goal_green"/>
...
class WX250PickPlaceImageEnv(gym.Env):
    ...
    def __init__(
        ...
        self.min_cube_goal_separation = 0.12  # bumped so cube doesn't spawn on the 10 cm marker
        self.goal_half = 0.05  # 10 cm x 10 cm square success region, matches visible marker
        self.max_steps = 250
        self.step_count = 0

        # Nominal values used by domain randomization to jitter around.
        self._nominal_cam_pos = np.array([0.70, 0.0, 0.38], dtype=np.float64)
    ...
    def _build_model(self, cube_xy: np.ndarray, goal_xy: np.ndarray) -> None:
        ...
        tmpdir = Path(tempfile.mkdtemp(prefix="wx250_img_mjcf_"))
    ...
    def _cache_ids(self) -> None:
        ...
        self.front_cam_id = mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_CAMERA, "front")
    ...
    def _randomize_scene(self) -> None:
        ...
        m.cam_pos[self.front_cam_id] = self._nominal_cam_pos + rng.uniform(-0.03, 0.03, size=3)

        # Light position jitter.
        m.light_pos[0] = self._nominal_light_pos + rng.uniform(-0.25, 0.25, size=3)

    def _get_pose_obs(self) -> np.ndarray:
        assert self.model is not None and self.data is not None
    ...
    def _get_image_obs(self) -> np.ndarray:
        ...
        self.obs_renderer.update_scene(self.data, camera="front")
    ...
    def _reward(self) -> tuple[float, RewardBreakdown, bool]:
        ...
        # Square success region matching the visible 10 cm x 10 cm goal marker.
        dx = abs(cube[0] - goal[0])
        dy = abs(cube[1] - goal[1])
        success = bool((dx < self.goal_half) and (dy < self.goal_half))
